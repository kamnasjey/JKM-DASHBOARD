      ...
      <div className="min-h-screen bg-background text-foreground">
        ...
        {result?.ok && result.summary && (
          <>
            ...
            {/* Tag Attribution Section */}
            {activeTags && activeTags.byTag.length > 0 && (
              <Card>
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        Performance attribution (min {activeTags.minTradesPerTag} trades)
                      </CardDescription>
                    </div>
                    {/* Mode Toggle */}
                    <div className="flex items-center gap-2 bg-muted/50 rounded-lg p-1">
                      <button
                        onClick={() => setTagsMode("any")}
                        className={cn(
                ...
                <CardContent>
                  ...
                  {(activeTags.topPositive.length > 0 || activeTags.topNegative.length > 0) && (
                    <div className="grid md:grid-cols-2 gap-4 mb-4">
                      {/* Top Positive */}
                      {activeTags.topPositive.length > 0 && (
                        <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-3">
                          ...
                          <div className="space-y-1.5">
                            {activeTags.topPositive.slice(0, 3).map((t) => (
                              <div key={t.tag} className="flex items-center justify-between text-sm">
                                <span className="flex items-center gap-1.5">
                                  <span className="font-mono">{t.tag}</span>
                                  {t.significant && (
                                    <span 
                                      className="text-[10px] px-1 py-0.5 rounded bg-green-500/20 text-green-500 font-medium cursor-help"
                                      title={`Statistically meaningful (n≥${activeTags.minTradesPerTag}, |Z|≥2)`}
                                    >
                                      ✓ sig
                                    </span>
                                  )}
                      ...
                      {/* Top Negative */}
                      {activeTags.topNegative.length > 0 && (
                        <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3">
                          ...
                          <div className="space-y-1.5">
                            {activeTags.topNegative.slice(0, 3).map((t) => (
                              <div key={t.tag} className="flex items-center justify-between text-sm">
                                <span className="flex items-center gap-1.5">
                                  <span className="font-mono">{t.tag}</span>
                                  {t.significant && (
                                    <span 
                                      className="text-[10px] px-1 py-0.5 rounded bg-red-500/20 text-red-500 font-medium cursor-help"
                                      title={`Statistically meaningful (n≥${activeTags.minTradesPerTag}, |Z|≥2)`}
                                    >
                                      ✓ sig
                                    </span>
                  ...
                  <div className="rounded-md border overflow-hidden">
                    <table className="w-full text-sm">
                      <thead className="bg-muted/50">
                        <tr>
                          <th className="text-left py-2 px-3 font-medium">Detector</th>
                          <th className="text-center py-2 px-3 font-medium">Trades</th>
                          <th className="text-center py-2 px-3 font-medium">Winrate</th>
                          <th 
                            className="text-center py-2 px-3 font-medium cursor-help"
                            title={tagsMode === "any" 
                              ? "Tag frequency: how often this tag appears (may exceed 100% due to multi-tag trades)" 
                              : "Trade share: percentage of total trades"
                            }
                          >
                            {tagsMode === "any" ? "Tag share" : "Trade share"}
                          </th>
                          <th className="text-right py-2 px-3 font-medium">Lift</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y">
                        {activeTags.byTag.map((t) => (
                          <tr key={t.tag} className="hover:bg-muted/30">
                            <td className="py-2 px-3">
                              <span className="flex items-center gap-1.5">
                                ...
                                {t.significant && (
                                  <span 
                                    className={cn(
                                      "text-[10px] px-1 py-0.5 rounded font-medium cursor-help",
                                      t.lift >= 0 ? "bg-green-500/20 text-green-500" : "bg-red-500/20 text-red-500"
                                    )}
                                    title={`Statistically meaningful (n≥${activeTags.minTradesPerTag}, |Z|≥2)`}
                                  >
                                    ✓
                            ...
                            <td className="text-center py-2 px-3">{t.trades}</td>
                            <td className="text-center py-2 px-3">
                              <span className={cn(
                                "font-medium",
                                t.winrate >= 0.5 ? "text-green-500" : 
                                t.winrate >= 0.35 ? "text-amber-500" : "text-red-500"
                              )}>
                                {(t.winrate * 100).toFixed(0)}%
                              </span>
                            </td>
                            ...
                            <td className="text-right py-2 px-3">
                              <span className={cn(
                                "font-medium",
                                t.lift > 0 ? "text-green-500" : t.lift < 0 ? "text-red-500" : "text-muted-foreground"
                              )}>
                                {t.lift > 0 ? "+" : ""}{(t.lift * 100).toFixed(0)}pp
                              </span>
                            </td>
                          </tr>
                        ))}
