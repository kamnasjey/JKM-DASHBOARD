  ...
  const response = {
    ...
    lastRun: lastDiag ? {
      ...
      response: {
        ...
        detectorClassification: {
          ...
          normalized: lastDiag.response.meta?.detectorsNormalized?.length ?? 0,
          ...
          requestedList: lastDiag.response.meta?.detectorsRequested ?? [],
          normalizedList: lastDiag.response.meta?.detectorsNormalized ?? [],
        ...
        explainability: lastDiag.response.explainability ? {
          rootCause: lastDiag.response.explainability.rootCause,
          explanation: lastDiag.response.explainability.explanation,
          severity: lastDiag.response.explainability.severity,
          suggestions: lastDiag.response.explainability.suggestions,
          debug: lastDiag.response.explainability.debug ?? null,
        } : null,
    ...
    docs: {
      ...
      fields: {
        ...
        "response.detectorClassification": "How backend classified your detectors (requested → normalized → implemented)",
        "response.backendVersion": "Backend SIM_VERSION - confirms which code is deployed",
        "response.explainability": "When entries=0, explains why no trades were found with debug info",
        "response.explainability.debug.hitsPerDetector": "Per-detector hit counts (which detectors fired)",
      },
    },
  }
