...
export async function POST(
  ...
  if (!API_KEY) {
    console.error("[proxy/active-strategy] Missing INTERNAL_API_KEY")
  ...
  try {
    ...
    const res = await fetch(`${BACKEND}/api/internal/user-data/active-strategy/${uid}`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      ...
      console.error("[proxy/active-strategy] Backend error:", res.status, text)
