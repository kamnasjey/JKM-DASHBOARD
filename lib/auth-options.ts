...
// Build providers list dynamically based on Prisma availability
function buildProviders() {
  const providers: NextAuthOptions["providers"] = [
    // Google OAuth always available (uses Firestore sync)
    GoogleProvider({
    ...
    async signIn({ user, account }) {
      ...
      if (account?.provider === "google" && user.email) {
        ...
        if (prisma) {
          ...
          console.log(`[Auth] Google login (Prisma disabled, skipping DB upsert): ${maskEmail(email)}`)
        }
      }
